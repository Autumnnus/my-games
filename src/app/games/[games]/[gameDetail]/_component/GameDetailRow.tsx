...
export default function GameDetailRow({ title, content }: DetailRowsProps) {
  ...
  const memorizedContent = useMemo(() => {
    ...
    if (title === 'platform') {
      return (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          <span style={{ display: 'inline-block', lineHeight: '20px' }}>{t(stringfied)}</span>
          <span
            style={{
              display: 'inline-flex',
              alignItems: 'center',
    ...
    return <Typography.Text type="secondary">{capitalizeFirstLetter(stringfied)}</Typography.Text>;
