'use client';
import { useAddGame, useIgdbGames } from '@/api/queries/useGames';
import usePlatforms from '@/hooks/usePlatforms';
import useGameDetailStore from '@/store/gameDetail';
import { IGDBGamesData, Status } from '@/types/games';
...
export default function AddGameModal() {
  const isOpen = useGameDetailStore(state => state.isAddGameModalOpen);
  const onClose = useGameDetailStore(state => state.toggleAddGameModal);
  ...
  const { mutateAsync: addMutate, isPending: addIsPending } = useAddGame();
  ...
  return (
    <Modal
      ...
      width={800}
      confirmLoading={addIsPending}
      cancelButtonProps={{ disabled: addIsPending }}
    >
        ...
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          {isIGDBAPIOpen ? (
            ...
            >
              ...
              >
                {igdbGames?.map(game => (
                  <Select.Option key={game.id} value={game.name} label={game.name}>
