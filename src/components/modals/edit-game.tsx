...
export default function EditGameModal() {
  ...
  const isEditGameModalOpen = useGameDetailStore(state => state.isEditGameModalOpen);
      ...
      >
        <Form.Item
          label={
            <Flex gap={10} align="center" justify="space-between">
              {t('game_name')}
              <Typography.Text type="secondary">{t('use_igdb_api')}</Typography.Text>
        ...
        >
          {isIGDBAPIOpen ? (
            ...
            <Input placeholder={gameNameLabel[randomNumber]} disabled={loading} />
          )}
        </Form.Item>
        ...
        <Flex gap={20}>
          <Form.Item label={t('game_play_time')} name="playTime" style={{ flex: 1 }}>
        ...
        <Form.Item label={t('game_review')} name="review">
          <Input.TextArea rows={4} placeholder={t('game_review')} disabled={loading} />
        </Form.Item>
